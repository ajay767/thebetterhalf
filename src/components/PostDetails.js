...
function PostDetails({ post, onClose }) {
  ...
  useEffect(() => {
    const fetchLikes = async () => {
      try {
        const { data } = await feed.getPost(post._id);
        // console.log(data);
        if (data.status) {
          const liked = data.post.likes.find(({ userId }) => {
    ...
    <Modal>
      <div className='bg-white w-11/12 md:w-8/12 mx-auto flex flex-col md:flex-row  '>
        <div className='relative md:w-5/12 h-[40vh]  md:h-[70vh]'>
          <span
            onClick={handleLike}
            className='h-10 w-10 absolute left-4 top-4 rounded-full  cursor-pointer   flex items-center justify-center'
          >
